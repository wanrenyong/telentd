/* Simple telnet server
   Copyright (C) 2013 ShangHai Sino-telecom Technology Inc.
*/
#define USE_SYSLOG 	1
//#define USE_ISSUE	1
...
static char loginpath[MAX_PATH]="/bin/bash";

/* shell name and arguments */

static char *argv_init[] = {"/bin/bash", NULL};
...
char *get_absolute_path(char *path)
{
    char *p;
    int count; 
    count = readlink( "/proc/self/exe", path, MAX_PATH ); 
...
static void usage()
{
	printf("Usage: telnetd [options]\n");
	printf("options:\n");
	printf("  -h     - show help\n");
	printf("  -p     - Port number, default is 23\n");
	printf("  -l     - Login path\n");
	printf("  -d     - Run in daeamon\n");
}
	...
	for (;;) {
		...
		switch (c) {
			...
			case 'd':
				...
				break;
			case 'l':
			    snprintf(loginpath, MAX_PATH, "%s", optarg);
			    break;
			case 'b':
...
//    snprintf(loginpath, MAX_PATH, "%s/vtysh", buf);
